...
data FloatOutSwitches = FloatOutSwitches {
  ...
  -- ^ Just n <=> float lambdas to top level, if doing so will abstract over n
  --              or fewer value variables
  --
  -- Nothing <=> float all lambdas to top level, regardless of how many free
  --             variables
  --
  -- Just 0 is the vanilla case: float a lambda iff it has no free vars.
  floatOutConstants :: Bool,
  -- ^ Float constants to top level, even if they do not escape a lambda.
  floatOutOverSatApps :: Bool,
  -- ^ Float out over-saturated applications based on arity information.
  --
